...
class IntrinsicRotator : public Rotator {
 ...
 public:
  ...
  IntrinsicRotator(emscripten::val v) {
    init(v["alpha"].isUndefined() ? 0. : v["alpha"].as<double>(),
         v["beta"].isUndefined() ? 0. : v["beta"].as<double>(),
         v["gamma"].isUndefined() ? 0. : v["gamma"].as<double>());
  }
#else
...
#ifdef __EMSCRIPTEN__
  ExtrinsicRotator(emscripten::val v) {
    init(v["gamma"].isUndefined() ? 0. : v["gamma"].as<double>(),
         v["beta"].isUndefined() ? 0. : v["beta"].as<double>(),
         v["alpha"].isUndefined() ? 0. : v["alpha"].as<double>());
  }
#else
