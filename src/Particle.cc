...
Particle::Particle(emscripten::val v)
    : type(v["type"].isUndefined()
      ...
      position(v["position"].isUndefined() ? Vector3<double>()
                                           : Vector3<double>(v["position"])),
      momentum(v["momentum"].isUndefined() ? Vector3<double>()
...
Particle::Particle(Lua &lua)
    : type(static_cast<ParticleType>(lua.getField("type", 0))),
      position(lua.getField("position", Vector3<double>())),
      momentum(lua.getField("momentum", Vector3<double>())),
      polarization(lua.getField("polarization", Vector3<double>())) {
  if (type == electron) {
    mass = 1.;
    charge = -1.;
    resetOpticalDepth();
