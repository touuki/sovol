...
template <typename T> Vector3<T> Vector3<T>::zero = Vector3();
...
template <typename T> inline Vector3<T> &Vector3<T>::operator*=(const T &c) {
    x *= c;
    y *= c;
    z *= c;
    return *this;
...
template <typename T> inline T Vector3<T>::dot(const Vector3 &v) const {
    return x * v.x + y * v.y + z * v.z;
};
...
template <typename T> inline T Vector3<T>::square() const {
    return x * x + y * y + z * z;
};
